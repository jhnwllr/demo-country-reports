...
create_elevation_map <- function(
  ...
  # Try multiple country code fields to find the country
  country_boundary <- countries[countries$ISO_A2 == cc, ]
  
  if (nrow(country_boundary) == 0) {
    # Try ISO_A2_EH field as backup
    country_boundary <- countries[countries$ISO_A2_EH == cc, ]
  }
  
  if (nrow(country_boundary) == 0) {
    # Try WB_A2 field as another backup
    country_boundary <- countries[countries$WB_A2 == cc, ]
  }
  
  if (nrow(country_boundary) == 0) {
    # Print available country codes for debugging
    available_codes <- unique(countries$ISO_A2[!is.na(countries$ISO_A2)])
    message("Available country codes: ", paste(sort(available_codes), collapse = ", "))
    stop(paste("Country code", cc, "not found. Check available codes above."))
  }
