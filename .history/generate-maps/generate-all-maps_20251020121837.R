...
# Build base map for debugging
basemap <- ggplot()

# Add country boundaries if available
if(nrow(countries_cropped) > 0) {
  basemap <- basemap + geom_sf(data = countries_cropped, fill = "gray95", color = "gray70", linewidth = 0.3)
}

# Add coastlines if available
if(nrow(coastlines_cropped) > 0) {
  basemap <- basemap + geom_sf(data = coastlines_cropped, color = "gray50", linewidth = 0.4)
}

# Add focal country boundary if available
if(nrow(focal_country) > 0) {
  basemap <- basemap + geom_sf(data = focal_country, fill = NA, color = "black", linewidth = 0.8)
}
