...
tryCatch({
  ...
  # Convert elevation to dataframe for ggplot and remove NAs
  elevation_df <- as.data.frame(elevation_raster, xy = TRUE, na.rm = TRUE)
  names(elevation_df) <- c("x", "y", "elevation")
  
  # Remove any remaining NAs and ensure we have valid data
  elevation_df <- elevation_df[complete.cases(elevation_df), ]
  
}, error = function(e) {
  warning("Could not retrieve elevation data: ", e$message)
  elevation_df <<- data.frame(x = numeric(0), y = numeric(0), elevation = numeric(0))
})

p <- ggplot() +
  # Add elevation as base layer (only if we have elevation data)
  {if(nrow(elevation_df) > 0) {
    list(
      geom_raster(data = elevation_df, aes(x = x, y = y, alpha = I(0.6)), 
                  fill = "gray80"),
  ...
  # Add species richness data with new fill scale
  ggnewscale::new_scale_fill() +
  geom_sf(data = dd, aes(fill = unique_species_count), color = NA, alpha = 0.8) +
  coord_sf(
...
p <- ggplot() +
  ...
  # Add coastlines
  geom_sf(data = coastlines_cropped, color = "gray50", size = 0.2) +
  # Add Chao1 data with new fill scale
  ggnewscale::new_scale_fill() +
  geom_sf(data = dd, aes(fill = chao1), color = NA, alpha = 0.8) +
  coord_sf(
    xlim = c(bbox["xmin"] - buffer_x, bbox["xmax"] + buffer_x),
    ylim = c(bbox["ymin"] - buffer_y, bbox["ymax"] + buffer_y),
    expand = FALSE, 
