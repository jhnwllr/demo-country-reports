...
# Add elevation as base layer if available
if(nrow(elevation_df) > 0) {
  p <- p + geom_raster(data = elevation_df, 
                       aes(x = x, y = y), 
...
p <- p +
  ...
  geom_sf(data = countries_cropped, fill = NA, color = "gray30", linewidth = 0.3) +
  # Add coastlines
  geom_sf(data = coastlines_cropped, color = "gray50", linewidth = 0.2) +
  # Add species richness data
  geom_sf(data = dd, aes(fill = unique_species_count), color = NA, alpha = 0.9) +
  coord_sf(
...
p <- p +
  ...
  geom_sf(data = coastlines_cropped, color = "gray50", linewidth = 0.2) +
  # Add Chao1 data
  geom_sf(data = dd, aes(fill = chao1), color = NA, alpha = 0.9) +
  coord_sf(
    xlim = c(bbox["xmin"] - buffer_x, bbox["xmax"] + buffer_x),
    ylim = c(bbox["ymin"] - buffer_y, bbox["ymax"] + buffer_y),
    expand = FALSE, 
