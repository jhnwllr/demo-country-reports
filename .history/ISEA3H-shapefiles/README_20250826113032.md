...
**resolution 1**
![resolution 1](grid_1.svg)

**resolution 6** 
![resolution 6](grid_6.svg)

**resolution 8**
![resolution 8](grid_8.png)

**resolution 9**
![resolution 9](grid_9.png)
...
> It is not possible to use the `dggridR` package directly with GBIF SQL downloads because GBIF uses **geogrid** ids instead of the **seqnum** used by `dggridR`. This repository acts as a bridge between the two. 
...
I used the `ISEA3H.R` script to generate the shapefiles. The script uses the `geogrid-dem-1.0-SNAPSHOT.jar` file from the `geogrid` project to create the mapping from **geodgrid_id** to **seqnum**. 

I would advise against generating higher resolution grids with `ISEA3H.R` and the `geogrid-dem-1.0-SNAPSHOT.jar` file unless absolutely necessary. Resolution 10 Will probably take several hours to run. Most likely a batch version of the java call would be needed to speed up the process.
