...
I would advise generating higher resolution grids my with `ISEA3H.R` and the `geogrid-dem-1.0-SNAPSHOT.jar` file unless absolutely necessary. Will probably take several hours to run. Most likely a batch version of the java call would be needed to speed up the process.
