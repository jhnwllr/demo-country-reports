...
dggs <- dgconstruct(projection = "ISEA", aperture = 3, topology = "HEXAGON", res = 7)
seqs <- dgrectgrid(dggs, minlat, minlon, maxlat, maxlon, cellsize = 0.05)$seqnum
grid_sf <- dgcellstogrid(dggs, seqs)


# === 1) Pick the grid resolution (integer; higher = finer) ===
res <- 6  # e.g., 7–12 are common
...
grid_sf$cell <- dgseqnum2cell(dggs, grid_sf$seqnum)
...
grid_sf = sf::st_read("species-count-maps/data/isea3h-shapefile/") |>
glimpse()
