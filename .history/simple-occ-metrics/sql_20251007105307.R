...
generate_ts_file <- function(data, filename) {
  ...
  ts_content <- "export const taxonomicGroups = [\n"
  
  for(i in 1:nrow(data)) {
    row <- data[i, ]
    group_name <- tools::toTitleCase(gsub("([a-z])([A-Z])", "\\1 \\2", row$taxon_group))
    color <- color_map[row$taxon_group] %||% "#999999"
  ...
  ts_content <- paste0(ts_content, "];\n\n")
  ts_content <- paste0(ts_content, "export interface TaxonomicGroup {\n")
  ts_content <- paste0(ts_content, "  group: string;\n")
  ts_content <- paste0(ts_content, "  occurrences: number;\n")
  ts_content <- paste0(ts_content, "  species: number;\n")
  ts_content <- paste0(ts_content, "  occurrenceGrowth: string;\n")
  ts_content <- paste0(ts_content, "  speciesGrowth: string;\n")
  ts_content <- paste0(ts_content, "  color: string;\n")
  ts_content <- paste0(ts_content, "}\n")
...
generate_ts_file(dd, "c:/Users/ftw712/Desktop/packages/CommunityMetrics/taxonomicGroups.ts")
...
generate_country_ts_files <- function(data) {
  ...
  for(country in countries) {
    ...
    # Write file
    filename <- paste0("c:/Users/ftw712/Desktop/packages/CommunityMetrics/taxonomicGroups", country, ".ts")
    writeLines(ts_content, filename)
