...
merged_data <- merge(d_2024, d_2025_only, by = "id", all = TRUE, suffixes = c("_2024", "_2025_only")) %>%
  dplyr::mutate(
    occurrence_growth_pct = round(((occurrence_count_2025 / occurrence_count_2024) * 100) - 100, 2),
    species_growth_pct = round(((distinct_species_count_2025 / distinct_species_count_2024) * 100) - 100, 2)
  ) %>%
