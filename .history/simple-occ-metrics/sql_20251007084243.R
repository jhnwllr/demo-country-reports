...
d <- occ_download_get('0056906-250920141307145') %>%
occ_download_import()

d_2024 <- d |> 
    dplyr::filter(year_group == "1600-2024") |>
    dplyr::mutate(id = paste0(countrycode,"_", taxon_group)) |> 
    glimpse()

d_2025 <- d |> 
    dplyr::filter(year_group == "1600-2025") |>
    dplyr::mutate(id = paste0(countrycode,"_", taxon_group)) |> 
    glimpse() 

d_2025_only <- d |> 
    dplyr::filter(year_group == "2025_only") |>
    dplyr::mutate(id = paste0(countrycode,"_", taxon_group)) |> 
    glimpse() 

merged_data <- merge(d_2024, d_2025_only, by = "id", all = TRUE, suffixes = c("_2024", "_2025_only")) %>%
  dplyr::mutate(
    occurrence_growth_pct = round(((occurrence_count_2025_only / occurrence_count_2024) * 100) - 100, 2),
    species_growth_pct = round(((distinct_species_count_2025_only / distinct_species_count_2024) * 100) - 100, 2)
  ) %>%
  # Handle cases where 2024 values are 0 or NA
  dplyr::mutate(
