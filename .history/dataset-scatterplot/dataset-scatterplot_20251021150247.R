...
merge(pp, by="datasetKey") |>
mutate(
  id = paste0(tolower(cc), "-", stringr::str_pad(row_number(), 3, pad = "0")),
  organization = datasetKey |> purrr::map_chr(~{
    dataset_info <- rgbif::dataset_get(.x)
    if(!is.null(dataset_info$publishingOrganizationTitle)) {
      dataset_info$publishingOrganizationTitle
    } else {
      "Unknown Organization"
    }
  }),
  description = datasetKey |> purrr::map_chr(~{
    dataset_info <- rgbif::dataset_get(.x)
    if(!is.null(dataset_info$description)) {
      stringr::str_trunc(dataset_info$description, 100)
    } else {
      "Dataset description not available"
    }
  }),
  publishingCountry = countryName,
  datasetUrl = paste0("https://www.gbif.org/dataset/", datasetKey),
  license = datasetKey |> purrr::map_chr(~{
    dataset_info <- rgbif::dataset_get(.x)
    if(!is.null(dataset_info$license)) {
...
for(i in seq_len(nrow(ddd))) {
  ...
  ts_content <- paste0(ts_content,
    ...
    "      organization: '", gsub("'", "\\\\'", dataset$organization), "',\n",
    "      description: '", gsub("'", "\\\\'", dataset$description), "',\n",
...
# Write to file
output_file <- paste0(countryCode, ".ts")
writeLines(ts_content, output_file)

cat("TypeScript file generated:", output_file, "\n")
cat("Preview of generated content:\n")
cat(substr(ts_content, 1, 500), "...\n")
