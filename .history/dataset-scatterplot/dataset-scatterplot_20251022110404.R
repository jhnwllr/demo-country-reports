...
for(i in seq_len(nrow(ddd))) {
  ...
  ts_content <- paste0(ts_content,
    ...
    "      publishedInCountry: ", ifelse(dataset$publishedInCountry == "true", "true", "false"), ",\n",
    "    }"
  )
...
# Write to file
output_dir <- "country-reports-demo-ui/data/dataset-scatterplot/"
if (!dir.exists(output_dir)) {
  dir.create(output_dir, recursive = TRUE)
}
output_file <- paste0(output_dir, cc, ".ts")
