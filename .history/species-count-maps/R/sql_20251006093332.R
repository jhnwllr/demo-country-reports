...
bbox <- st_bbox(dd)
buffer_x <- (bbox["xmax"] - bbox["xmin"]) * 0.1  # 10% buffer on each side
buffer_y <- (bbox["ymax"] - bbox["ymin"]) * 0.1  # 10% buffer on each side
...
p <- ggplot() +
  ...
  theme(
    ...
    plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm"), # Add some plot margins
