      ...
      <svg width={width} height={height} className="border border-gray-200 rounded-lg">
        ...
        <g transform={`translate(${margin.left},${margin.top})`}>
          ...
          {groupData.data.map(point => (
            ...
            >
              <title>
                {point.year}: {point.cumulativeSpecies.toLocaleString()} species
                {'\n'}Circle size represents {point.effort.toLocaleString()} occurrence records published in {point.year}
              </title>
            </circle>
          ))}
